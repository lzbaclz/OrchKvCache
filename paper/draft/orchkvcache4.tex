...
We implement OrchKvCache in ${\sim}$4,500 lines of C/CUDA plus ${\sim}$1,200 lines of Python with pybind11 bindings. Evaluation on four models (7B GQA through 13B MHA) with NVIDIA A100-80GB demonstrates: (i) \textbf{139--597$\times$} fewer migrations than FIFO, translating to \textbf{1.28--1.77$\times$} throughput in the prototype framework and \textbf{1.08--1.15$\times$} in vLLM's production engine under memory pressure; (ii) sub-60\,$\mu$s scheduling latency at 4,096 blocks with sub-linear scaling; (iii) GPU$\leftrightarrow$DRAM transfer at 23\,GB/s saturating PCIe Gen4; and (iv) bit-exact lossless output on all four models. A systematic overhead decomposition (\S\ref{sec:overhead}) separates the scheduling algorithm's effectiveness from prototype implementation overhead.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.92\columnwidth,clip]{../benchmarks/paper_figures/fig1_throughput_4models.pdf}
...
\begin{figure}[b]
\centering
\includegraphics[width=0.95\columnwidth,clip]{../benchmarks/paper_figures/fig2_eviction_comparison.pdf}
...
\begin{figure}[t]
\centering
\subfloat[TPOT vs.\ request count. FIFO degrades under scaling; OrchKvCache remains stable.\label{fig:tpot}]{%
  \includegraphics[width=0.48\columnwidth,clip]{../benchmarks/paper_figures/fig3_tpot_stability.pdf}}
\hfill
\subfloat[Scheduling scalability. Latency scales sub-linearly; P99 $<$ 60\,$\mu$s at 4,096 blocks.\label{fig:scalability}]{%
  \includegraphics[width=0.48\columnwidth,clip]{../benchmarks/paper_figures/fig10_scalability.pdf}}
\caption{(a) Per-token latency stability under increasing concurrency. (b) Scheduler latency from 64 to 4,096 blocks.}
\label{fig:tpot-scalability}
\end{figure}
...
\begin{figure}[b]
\centering
\includegraphics[width=0.95\columnwidth,clip]{../benchmarks/paper_figures/fig4_speedup_heatmap.pdf}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.82\columnwidth,clip]{../benchmarks/paper_figures/fig6_quality_all_models.pdf}
...
\begin{figure}[b]
\centering
\includegraphics[width=0.95\columnwidth,clip]{../benchmarks/paper_figures/fig5_ablation_4models.pdf}
...
\begin{figure}[t]
\centering
\subfloat[Classification accuracy. At $\alpha \geq 0.7$, the hot ratio matches ground truth (25\%).\label{fig:policy}]{%
  \includegraphics[width=0.48\columnwidth,clip]{../benchmarks/paper_figures/fig9_policy_heatmap.pdf}}
\hfill
\subfloat[Prefetch dispatch saturates at $K \geq 8$; overhead is 5.7--5.9\,$\mu$s/step.\label{fig:prefetch}]{%
  \includegraphics[width=0.48\columnwidth,clip]{../benchmarks/paper_figures/fig12_prefetch.pdf}}
\caption{(a) Hot-cold classification accuracy across weight configurations. (b) Prefetch dispatch effectiveness and scheduling overhead.}
\label{fig:policy-prefetch}
\end{figure}
...
\begin{figure}[tbp]
\centering
\includegraphics[width=0.95\columnwidth,clip]{../benchmarks/figures/fig17_hyperparam_sensitivity.pdf}
...
\begin{figure}[t]
\centering
\subfloat[Throughput. OrchKvCache outperforms FIFO, with larger gains on LLaMA-2-7B (MHA, 512\,KB/tok).\label{fig:w3-throughput}]{%
  \includegraphics[width=0.48\columnwidth,clip]{../benchmarks/paper_figures/fig_w3_throughput.pdf}}
\hfill
\subfloat[Eviction counts (log scale). OrchKvCache reduces migrations by 123--374$\times$.\label{fig:w3-eviction}]{%
  \includegraphics[width=0.48\columnwidth,clip]{../benchmarks/paper_figures/fig_w3_eviction.pdf}}
\caption{Realistic workload evaluation. (a) Throughput under ShareGPT-like and LongContext-mix distributions. (b) Eviction counts showing 123--374$\times$ reduction.}
...
\begin{figure}[b]
\centering
\includegraphics[width=0.82\columnwidth,clip]{../benchmarks/paper_figures/fig_w4_ssd_tier.pdf}
...
\midrule
\multicolumn{2}{@{}l}{\emph{Correctness and storage:}} \\
...
\textbf{Limitations.} (1)~The throughput gap between OrchKvCache and GPU-only is dominated by Python-level orchestration; the overhead decomposition (\S\ref{sec:overhead}) shows the Python scheduling loop adds 31.8--112\,ms per step while the C classifier runs in 2.6--4.4\,$\mu$s---a ${\sim}$4{,}000$\times$ gap that is purely an implementation artefact.  (2)~The 1.28--1.77$\times$ throughput improvement (\S\ref{sec:e2e}) is measured in a regime where shared framework overhead amplifies the relative difference; the vLLM integration (1.08--1.15$\times$, Table~\ref{tab:vllm-block})---which runs within a production C++ engine---provides a more conservative but implementation-independent estimate.  (3)~The prefetch scheduler uses EMA-based prediction; cross-layer~\cite{infinigen} or query-aware~\cite{quest} signals could improve accuracy.  (4)~Our prototype requires \texttt{output\_attentions=True} for eager attention, which materialises the full attention matrix and triggers OOM at 8K+ tokens on A100-80GB; integrating with FlashAttention's tiled output or using the QK-norm proxy (\S3.3) would remove this constraint.  (5)~Our prototype processes requests in round-robin fashion; the vLLM experiment validates the benefit under true continuous batching.

%% ====================================================================
\section{Conclusion}
%% ====================================================================

We have presented OrchKvCache, a tiered KV-cache management system driven by runtime attention-derived hotness classification. The core scheduling algorithm reduces unnecessary data migrations by \textbf{139--597$\times$} over FIFO, achieves a competitive ratio within \textbf{1.51$\times$} of the offline optimal (Belady), and guarantees \textbf{100\% lossless} output through the full GPU$\to$DRAM$\to$SSD$\to$DRAM$\to$GPU migration path across four models (7B GQA through 13B MHA).
