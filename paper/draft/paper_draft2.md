...
> **Format: 10 pages + references (ACM format)**
...
<!--
[FIGURE 1: KV-cache size vs. sequence length for 5 representative models]
描述: 折线图。X 轴: 序列长度 (512 – 128K, log scale)；Y 轴: KV cache 大小 (GB, log scale)。
5 条线分别对应 LLaMA-2-7B, LLaMA-2-13B, LLaMA-2-70B, LLaMA-3-8B, Qwen2-7B。
水平虚线标注 A100-80GB 容量、A6000-48GB 容量。
灰色阴影区域标注 "KV cache > model weight"。
数据来源: experiments/exp1_motivation/results/m1_kvcache_theory.json (part_a_size_vs_seqlen)
-->
...
<!--
[TABLE 1: KV cache sizes and memory analysis]
描述: 扩展版表格，包含 7 个模型 × 9 个序列长度。
列: Model | Attention Type | n_kv_heads | Weight (GB) | Per-token (MB) | 4K | 8K | 16K | 32K | 64K | 128K | KV≥Weight crossover
粗体标注 KV + Weight > 80GB 的情况。
最后一列给出 KV 超过 weight 的 crossover 序列长度。
数据来源: m1_kvcache_theory.json (part_a + part_d_crossover)
-->
...
<!--
[TABLE 2: Maximum batch size on A100-80GB]
描述: 表格。
列: Model | seq_len=2K | 4K | 8K | 16K | 32K | 128K
行: LLaMA-2-7B, LLaMA-2-13B, LLaMA-3-8B, Mistral-7B, Qwen2-7B
"0" 表示连单请求都放不下。
数据来源: m1_kvcache_theory.json (part_c_max_batch)
-->
...
<!--
[TABLE 3: Attention distribution statistics across layers and input lengths]
描述: 表格。
行组按 input length 分 (512 / 1024 / 2048)；
列: Input Length | Layer Range | Avg Top-5% Coverage | Avg Top-10% Coverage | Avg Top-20% Coverage | Avg Gini
数据来源: m2_attention_analysis.json, 按 layer 范围聚合 (layers 0-6, 7-13, 14-20, 21-27, all)
-->
...
<!--
[FIGURE 2: Attention CDF at token-level and block-level]
描述: 双折线图。X 轴: Top-K% (5, 10, 20, 50, 100); Y 轴: 累积注意力权重 (0–100%)。
两条线: "Token-level" 和 "Block-level (block_size=16)"，取所有层平均值。
三组线对应 short/medium/long input。
附属图: 各层 Gini 系数的 bar chart (X: layer index 0–27, Y: |Gini|)。
数据来源: m2_attention_analysis.json
-->
...
<!--
[FIGURE 3: Attention sink — per-layer first-token attention fraction]
描述: 双柱状图。X 轴: Layer index (0–27); Y 轴: 注意力百分比 (0–80%)。
两组柱: "First token only" (深色) 和 "First 5 tokens" (浅色)。
仅展示 seq_len=512 的数据 (其他 seq_len 趋势一致)。
在 Layer 4 处标注峰值 "77.3%"。
数据来源: m2_attention_analysis.json, layer_stats.first_token_attn_pct / first_5_tokens_attn_pct
-->
...
<!--
[TABLE 4: Per-tier bandwidth at various transfer sizes]
描述: 同 draft1 的 Table 2，但增加一列 "Latency (μs)" 和一行 "256MB"。
对于 GPU D2D / GPU↔DRAM / DRAM copy / SSD read / SSD write 分别给出。
在表下方标注: GPU D2D 数据来自 cudaMemcpy within device; GPU↔DRAM 使用 pinned memory + cudaMemcpyAsync; SSD 使用 direct I/O pread/pwrite。
数据来源: m4_tier_comparison.json
-->
...
<!--
[FIGURE 4: SSD bandwidth utilization under different eviction strategies]
描述: 分组柱状图。X 轴: 配置 (vLLM blk16 ×1/×8/×32 layers, blk64 ×1/×8/×32, MHA32 等)。
Y 轴: SSD 写带宽利用率 (%)。
每组两柱: Naive eviction (红) 和 Batched eviction (蓝)。
水平虚线标注 SSD 峰值写带宽 (5.3 GB/s)。
数据来源: m3_io_efficiency.json
-->
...
<!--
[FIGURE 5: Storage hierarchy and I/O path overview]
描述: 概念图。从上到下画 4 层:
  Layer 0: GPU HBM (80 GB, ~274 GB/s D2D) — 标注 "Hot blocks"
  Layer 1: Host DRAM (256 GB, 23 GB/s via PCIe) — 标注 "Warm blocks"
  Layer 2: NVM (e.g., 128 GB Optane PM, ~300 ns random) — 标注 "Recently-cold blocks, small writes"
  Layer 3: SSD (e.g., 3.84 TB NVMe, 17.8 GB/s seq read) — 标注 "Cold blocks, batch writes"
各层之间画箭头标注 demote/promote 操作和对应带宽。
右侧标注 capacity 从小到大、latency 从低到高的趋势。
-->
...
<!--
[FIGURE 6: OrchKvCache system architecture]
描述: 系统架构图，分三个纵向区域:
左侧 "LLM Engine (vLLM)": Attention Kernel → attention scores; Block Allocator → alloc/free events
中间 "OrchKvCache Scheduler":
  C1 Attention Tracker ──→ C2 Hot-Cold Classifier ──→ C3 Adaptive Threshold
                                    │
                           C4 Eviction Policy ←─── GPU/DRAM watermark feedback
                           C5 Prefetch Scheduler
                           C6 Pipeline Coordinator
                                    │
                           C7 Migration Engine ──→ transfer_fn callbacks
                           C8 Tiered Manager (统一调度入口)
右侧 "Storage Tiers" (从上到下):
  T0: GPU HBM (slab pool, cudaMalloc)
  T1: Host DRAM (pinned slab pool, cudaMallocHost)
  T2: NVM (OrchFS page writes, 4KB)
  T3: SSD (OrchFS aligned block writes, 32KB)
箭头:
  T0 ←→ T1: cudaMemcpyAsync (transfer_engine, multi-stream)
  T1 ←→ T2/T3: pwrite/pread (io_worker_pool, async thread pool)
  T0 → T3: two-hop via T1 staging buffer
-->
...
<!--
[FIGURE 7: KV block state machine]
描述: 状态机图。
五个节点: FREE → ALLOCATED → ACTIVE ↔ MIGRATING → EVICTED → (回到 ACTIVE via promote)
标注每个转移:
  FREE → ALLOCATED: orchkv_request_create → alloc_blocks_for_layer
  ALLOCATED → ACTIVE: orchkv_prefill / orchkv_append_token (数据写入完成)
  ACTIVE → MIGRATING: migration_engine 开始 demote (acquire wrlock)
  MIGRATING → EVICTED: demote 完成, 释放 GPU/DRAM slot, 更新 tier
  EVICTED → MIGRATING: promote 开始 (acquire wrlock)
  MIGRATING → ACTIVE: promote 完成, 数据回到 GPU, 释放 wrlock
  ACTIVE → MIGRATING → ACTIVE: prefetch cycle (无感知, wrlock 保护)
特殊:
  KV_FLAG_PIN / KV_FLAG_ATTN_SINK 的 block 永远跳过 ACTIVE→MIGRATING
  MIGRATING 状态下 rdlock 阻塞, 保证不读到部分数据
-->
...
<!--
[FIGURE 8: Three-stage pipeline timeline]
描述: 时间线图 (类似 Gantt chart):
  行1 "GPU Compute": Step N (长条), Step N+1, Step N+2 ...
  行2 "DRAM→GPU Transfer": [空] | Prefetch for N+1 (与 Step N 计算重叠) | Prefetch for N+2 ...
  行3 "SSD→DRAM Preload": [空] | [空] | Preload for N+2 (与 Step N 计算重叠) ...
用虚线标注 step boundary。
标注: "Overlap region" 在 compute 与 transfer 并行的区域用阴影。
Pipeline latency formula:
  effective_step = max(compute, transfer) ≈ compute (if transfer < compute)
-->
...
<!--
[FIGURE 9: fig08_policy_heatmap.png + fig09_classification_distribution.png]
描述: 左图: 热力图，X 轴 α 值，Y 轴 access pattern，色值为 hot ratio。
右图: 分组柱状图，每组三柱 hot/warm/cold 的 block 数量。
已生成: benchmarks/figures/fig08_policy_heatmap.png, fig09_classification_distribution.png
-->
...
<!--
[FIGURE 10: fig11_prefetch_dispatches.png + fig12_prefetch_latency.png]
描述: 左图: budget vs dispatch count (折线图，在 K=8 处标注 saturation)。
右图: budget vs avg/P99 scheduling latency (双折线)。
已生成: benchmarks/figures/fig11_prefetch_dispatches.png, fig12_prefetch_latency.png
-->
...
<!--
[FIGURE 11: fig13_storage_bandwidth.png]
描述: 四条折线: GPU→DRAM, DRAM→GPU, DRAM→Storage(W), Storage→DRAM(R)。
X 轴: transfer size (log); Y 轴: bandwidth (GB/s)。
已生成: benchmarks/figures/fig13_storage_bandwidth.png
-->
...
<!--
[FIGURE 12: fig14_scalability.png]
描述: Log-log 散点图 + 拟合线。X 轴: n_blocks; Y 轴: avg scheduling latency (μs)。
标注 power-law exponent = 0.749 和 per-block cost = 9.36 ns。
已生成: benchmarks/figures/fig14_scalability.png
-->
...
<!--
[FIGURE 13: fig01_throughput_vs_seqlen.png + fig02_throughput_vs_batchsize.png]
描述: 左图: 吞吐 vs seq_len, 两条线 baseline/orchkv, 多 batch_size 用不同标记。
右图: 吞吐 vs batch_size, 两条线, 多 seq_len。
已生成: benchmarks/figures/fig01_throughput_vs_seqlen.png, fig02_throughput_vs_batchsize.png
-->
...
<!--
[FIGURE 14: fig05_latency_breakdown.png]
描述: 堆叠柱图, baseline vs orchkv, 分段为 TTFT / decode latency / overhead。
已生成: benchmarks/figures/fig05_latency_breakdown.png
-->
...
### 5.4 Storage Tier Ablation and Capacity Extension
...
<!--
[FIGURE 15: fig06_tier_throughput.png + fig07_tier_gpu_memory.png]
描述: 左图: 柱状图, 4 种配置的吞吐量 (差异 < 0.5%)。
右图: 柱状图, 4 种配置的 GPU 内存使用。
已生成: benchmarks/figures/fig06_tier_throughput.png, fig07_tier_gpu_memory.png
-->

**Key finding.** Under the current scenario (A100-80GB, 7B model, seq\_len=4096), GPU memory is not the bottleneck—all configurations fit comfortably. The throughput variation across all four configurations is **< 0.53%** (6,050.6 to 6,082.7 tok/s), confirming that the additional tier management introduces no measurable performance regression. The 4-tier configuration even slightly outperforms the 3-tier, likely due to measurement variance.
...
<!--
[FIGURE 16: fig10_block_size_ablation.png]
描述: 柱状图, X: block_size, Y: throughput。
已生成: benchmarks/figures/fig10_block_size_ablation.png
-->
...
<!--
[FIGURE 17: tab01_quality_verification.png]
描述: 表格式可视化, 左列 baseline PPL, 右列 orchkv PPL, 差值列全为 0.0000。
已生成: benchmarks/figures/tab01_quality_verification.png
-->
...
| 编号 | 类型 | 内容 | 位置 | 数据来源 | 状态 |
|------|------|------|------|---------|------|
| **Fig. 1** | 折线图 | KV cache size vs. seq_len for 5 models | §2.1 | m1_kvcache_theory.json | **需生成** |
| **Tab. 1** | 表格 | KV cache sizes + crossover point | §2.1 | m1_kvcache_theory.json | 文中已写 |
| **Tab. 2** | 表格 | Max batch size on A100-80GB | §2.1 | m1_kvcache_theory.json (part_c) | **需生成** |
| **Tab. 3** | 表格 | Attention distribution stats | §2.3 | m2_attention_analysis.json | **需生成** |
| **Fig. 2** | 双折线图 + bar | Attention CDF + Gini per layer | §2.3 | m2_attention_analysis.json | **需生成** |
| **Fig. 3** | 双柱状图 | Attention sink per layer | §2.3 | m2_attention_analysis.json | **需生成** |
| **Tab. 4** | 表格 | Per-tier bandwidth | §2.4 | m4_tier_comparison.json | 文中已写 |
| **Fig. 4** | 分组柱状图 | SSD bandwidth utilization | §2.4 | m3_io_efficiency.json | **需生成** |
| **Fig. 5** | 概念图 | 4-tier hierarchy overview | §2.5 | — | **需手绘** |
| **Fig. 6** | 架构图 | OrchKvCache system architecture | §3.1 | — | **需手绘** |
| **Fig. 7** | 状态图 | KV block state machine | §3.2 | — | **需手绘** |
| **Fig. 8** | Gantt chart | Three-stage pipeline timeline | §3.6 | — | **需手绘** |
| **Tab. 5** | 表格 | Experimental configuration | §5.1 | — | 文中已写 |
| **Tab. 6** | 表格 | E5 classification results | §5.2 | benchmark_e5_policy_sweep.json | 文中已写 |
| **Fig. 9** | 热力图+柱状图 | E5 policy heatmap + distribution | §5.2 | fig08/fig09 | ✅ 已生成 |
| **Tab. 7** | 表格 | E7 prefetch dispatch vs budget | §5.2 | benchmark_e7_prefetch.json | 文中已写 |
| **Fig. 10** | 折线图 | E7 dispatch + latency vs budget | §5.2 | fig11/fig12 | ✅ 已生成 |
| **Tab. 8** | 表格 | E8 inter-tier bandwidth | §5.2 | benchmark_e8_storage_bw.json | 文中已写 |
| **Fig. 11** | 多折线图 | E8 bandwidth vs transfer size | §5.2 | fig13 | ✅ 已生成 |
| **Tab. 9** | 表格 | E9 scheduling scalability | §5.2 | benchmark_e9_scalability.json | 文中已写 |
| **Fig. 12** | Log-log图 | E9 latency vs block count | §5.2 | fig14 | ✅ 已生成 |
| **Tab. 10** | 表格 | E1 throughput comparison | §5.3 | benchmark_e2e.json | 文中已写 |
| **Fig. 13** | 折线图 | E1 throughput plots | §5.3 | fig01/fig02 | ✅ 已生成 |
| **Tab. 11** | 表格 | E3 latency breakdown | §5.3 | test_latency_breakdown.json | 文中已写 |
| **Fig. 14** | 堆叠柱图 | E3 latency breakdown | §5.3 | fig05 | ✅ 已生成 |
| **Tab. 12** | 表格 | E4 tier ablation | §5.4 | benchmark_e4_tier_ablation.json | 文中已写 |
| **Fig. 15** | 柱状图 | E4 tier throughput + memory | §5.4 | fig06/fig07 | ✅ 已生成 |
| **Tab. 13** | 表格 | E6 block size ablation | §5.4 | benchmark_e6_block_size.json | 文中已写 |
| **Fig. 16** | 柱状图 | E6 block size ablation | §5.4 | fig10 | ✅ 已生成 |
| **Tab. 14** | 表格 | E10 quality verification | §5.5 | eval_e10_quality.json | 文中已写 |
| **Fig. 17** | 表格可视化 | E10 quality table | §5.5 | tab01 | ✅ 已生成 |
| **Tab. 15** | 表格 | Results summary | §5.6 | — | 文中已写 |
...
**Compression as an orthogonal optimization.** OrchKvCache's lossless block migration is orthogonal to KV-cache compression. Applying KIVI's 2-bit quantization [32] before writing to NVM/SSD would reduce storage footprint by ~4× and proportionally decrease transfer latency—without changing the tiered management logic. Similarly, CacheGen's [6] custom tensor encoding (3–5× compression) could be applied at tier boundaries. We consider this an engineering optimization rather than a research contribution and leave it for future work.

**Limitations and future work.** We acknowledge three limitations of the current evaluation. First, the Qwen2.5-7B model on A100-80GB does not create sufficient GPU memory pressure to trigger frequent evictions—the core capacity-extension benefit is demonstrated only theoretically (Table 1) rather than empirically. Future work will evaluate on memory-constrained configurations (e.g., LLaMA-2-13B at 32K+ context, or by artificially limiting `gpu_memory_utilization`). Second, the vLLM integration currently simulates tiered management via `swap_space` configuration rather than routing through the full C/CUDA migration path; a complete KVConnector implementation for vLLM 0.8+ is in progress. Third, the prefetch scheduler uses a simple EMA-based predictor; incorporating InfiniGen's [5] cross-layer prediction or Quest's [9] query-aware signals could improve prefetch accuracy.
