...
\textbf{Demote path.} When C4 identifies victim blocks:
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/fig02_eviction_comparison.pdf}
...
\begin{figure*}[t]
\centering
\subfloat[Classification accuracy.\label{fig:policy}]{%
...
\caption{Scheduling subsystem microbenchmarks. (a)~Classification accuracy across $\alpha$--$\beta$ weight space. (b)~Prefetch dispatch count and per-step latency vs.\ budget $K$. (c)~Scheduler latency from 64 to 4,096 blocks. (d)~Inter-tier bandwidth; GPU$\leftrightarrow$DRAM saturates PCIe Gen4 at 23\,GB/s.}
...
\textbf{Prefetch effectiveness.} Figure~\ref{fig:prefetch} shows dispatch saturation at budget $K \geq 8$ (245 dispatches per 100 steps) with per-step overhead of only 5.7--5.9\,$\mu$s---negligible relative to 1--10\,ms decode steps.
...
\subfloat[Throughput.\label{fig:w3-throughput}]{%
  \includegraphics[width=0.48\columnwidth,clip]{figures/fig19_realistic_throughput.pdf}}\hfill
\subfloat[Eviction counts.\label{fig:w3-eviction}]{%
  \includegraphics[width=0.48\columnwidth,clip]{figures/fig20_realistic_eviction.pdf}}
...
Table~\ref{tab:selective} shows that even the \textbf{top-1\%} of blocks ranked by OrchKvCache's EMA score---a single block of 16 tokens out of ${\sim}$64---captures \textbf{100\%} of the non-sink attention weight.  Random ranking at the same budget captures only 2\%.  This extreme concentration means a kernel-level selective restore would need to fetch \emph{at most a few blocks} to cover all attention-critical data; the remaining 99\% of blocks could be deferred or skipped entirely, reducing restore volume by up to \textbf{100$\times$} with zero quality loss.  The EMA's advantage over random is not merely quantitative (100\% vs.\ 2\%) but qualitative: it identifies the \emph{exact} blocks that will receive attention, validating the scheduling algorithm as a precise predictor for kernel-level block placement.
...
Table~\ref{tab:summary} summarizes all key findings.
...
\midrule
\multicolumn{2}{@{}l}{\emph{Overhead analysis (\S\ref{sec:overhead}):}} \\
...
We have presented OrchKvCache, a tiered KV-cache management system driven by runtime attention-derived hotness classification. Across four models (7B--13B), OrchKvCache achieves \textbf{1.28--1.77$\times$} throughput over FIFO with \textbf{139--597$\times$} fewer migrations, \textbf{1.53$\times$} speedup under realistic workloads, and \textbf{100\% lossless} output through the full GPU$\to$DRAM$\to$SSD path. Integration into vLLM confirms the benefit in production: \textbf{1.12$\times$} throughput via block-level victim scoring.

Overhead decomposition shows the scheduling \emph{algorithm} is effective (native classifier latency: $<$\,40\,$\mu$s); the remaining gap is dominated by Python orchestration overhead (23--49\,ms), resolvable by porting the scheduling loop to the native runtime. Combined with prefix-KV systems such as IMPRESS~\cite{impress} for the loading phase, OrchKvCache completes the full decode-phase KV-cache lifecycle.
