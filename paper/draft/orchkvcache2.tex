...
\textbf{Setup.} LLaMA-2-7B (MHA-32, 512\,KB/token) on A100-80GB with \texttt{gpu\_memory\_utilization}=0.25 (874 GPU KV blocks, $\sim$6.8\,GB KV budget), \texttt{swap\_space}=32\,GB, 16 concurrent requests of $\sim$1,500 tokens each, 64 output tokens. This configuration triggers exactly one preemption event per run.

\begin{table}[t]
\centering
\caption{vLLM integration: three victim-selection policies under memory pressure (LLaMA-2-7B, gpu\_util=0.25, 16 requests).}
\label{tab:vllm-block}
\small
\begin{tabular}{@{}lccc@{}}
\toprule
Policy & Throughput (tok/s) & Preemptions & Speedup vs FIFO \\
\midrule
FIFO (vLLM default)   & 217.9 & 1 & 1.00$\times$ \\
Progress-aware        & 240.9 & 1 & 1.11$\times$ \\
Block-level scoring   & \textbf{243.4} & 1 & \textbf{1.12$\times$} \\
\bottomrule
\end{tabular}
\end{table}

\textbf{Result.} Table~\ref{tab:vllm-block} shows that block-level scoring achieves \textbf{1.12$\times$} throughput over FIFO, while progress-aware reaches 1.11$\times$. All three policies trigger exactly one preemption, but they select \emph{different} victims---the quality of the choice determines the throughput impact. Block-level scoring picks the sequence whose KV blocks have the lowest aggregate attention, minimizing disruption to hot sequences. This confirms that even within the constraint of request-level swap, finer-grained scoring produces better outcomes.
...
\midrule
\multicolumn{2}{@{}l}{\emph{vLLM integration:}} \\
Block scoring vs FIFO     & \textbf{1.12$\times$} \\
Intra-request partial swap& Infeasible \\
\midrule
...
\textbf{Quality under memory limits.} Table~\ref{tab:vs-infinigen} compares perplexity on WikiText-2 (seq\_len=2048) when GPU KV cache capacity is limited to 80\% of full. InfiniGen's Counter-based eviction policy achieves near-lossless perplexity (5.69 on LLaMA-2-7B), but its FIFO baseline degrades catastrophically (22.26). We verify OrchKvCache's lossless guarantee experimentally: at 80\% GPU budget on LLaMA-2-7B, OrchKvCache offloads 33,280 block-layer pairs to CPU DRAM and achieves perplexity \textbf{5.8230}---a $5.7{\times}10^{-5}$ deviation from the full-cache baseline (5.8229), entirely attributable to FP16 rounding during GPU$\leftrightarrow$DRAM transfer rather than information loss.

\begin{table}[t]
\centering
\caption{Perplexity comparison on WikiText-2 (seq=2048). InfiniGen data from~\cite{infinigen} Table~2. OrchKvCache perplexity measured on A100-80GB with 80\% GPU budget and DRAM offloading.}
\label{tab:vs-infinigen}
\small
\begin{tabular}{@{}lcc@{}}
\toprule
Scheme & LLaMA-2-7B & LLaMA-2-13B \\
\midrule
Full Cache (100\%) & 5.82 / 5.69$^\dagger$ & 5.25$^\dagger$ \\
80\% FIFO eviction~\cite{infinigen} & 22.26$^\dagger$ & 21.41$^\dagger$ \\
80\% Counter (InfiniGen)~\cite{infinigen} & 5.69$^\dagger$ & 5.25$^\dagger$ \\
\textbf{OrchKvCache (80\% GPU)} & \textbf{5.82} & --- \\
\bottomrule
\multicolumn{3}{@{}l}{\footnotesize $^\dagger$Reported by~\cite{infinigen}; slight difference in Full Cache}\\
\multicolumn{3}{@{}l}{\footnotesize ~~values due to different transformers versions/eval samples.}\\
\end{tabular}
\end{table}
...
\textbf{Throughput.} Direct throughput comparison is not possible because InfiniGen is built on FlexGen~\cite{flexgen} (OPT models, single-GPU offloading) while OrchKvCache uses HuggingFace transformers and vLLM. InfiniGen reports up to 3.00$\times$ speedup over FlexGen on OPT-13B; OrchKvCache achieves 1.28--1.77$\times$ over FIFO on LLaMA-2-7B through 13B. The two systems target different bottlenecks: InfiniGen reduces \emph{prefetch volume} by predicting which blocks to load; OrchKvCache reduces \emph{migration frequency} by keeping hot blocks on GPU and batching cold-block I/O. These optimizations are orthogonal and could be combined in a unified system.

%% ====================================================================
\section{Related Work}
%% ====================================================================

\textbf{KV-Cache Management.} vLLM~\cite{vllm} introduced PagedAttention for KV-cache paging. FlexGen~\cite{flexgen} formulated offline three-tier offloading. InfiniGen~\cite{infinigen} pioneered cross-layer prefetching with speculative KV selection, achieving up to 3$\times$ speedup on OPT-13B; we provide a detailed comparison in \S\ref{sec:infinigen-comparison}. vTensor~\cite{vtensor} provides virtual tensor management. Mooncake~\cite{mooncake} deploys datacenter-scale KV-cache pooling. Infinite-LLM~\cite{infinitellm} distributes KV-cache across GPU clusters. OrchKvCache adds \emph{attention-driven, per-block classification} with heterogeneous IO adaptation.
...
We profile the overhead gap by varying the attention sampling interval $N$ on Qwen2.5-7B (Table~\ref{tab:sampling}). Pure SDPA (no KV management) achieves 1,658\,tok/s; enabling OrchKvCache with $N{=}1$ (every-step eager attention) drops to 180\,tok/s---a 9.2$\times$ gap. Crucially, increasing $N$ to 50 does not improve throughput (still ${\sim}$183\,tok/s), because the dominant cost is not attention weight extraction itself but the Python-level block reconstruction and per-step scheduling loop. Classification accuracy (measured via trace simulation in Part~A) remains stable at 80--88\% for $N \in [2, 20]$, confirming that sparse sampling preserves scheduling quality. These results indicate that the path to closing the throughput gap is \emph{not} sampling frequency but rather (a)~integrating the scheduler into FlashAttention's tiled execution (avoiding block reconstruction) and (b)~moving the scheduling loop to C/CUDA (eliminating Python overhead).
...
\textbf{Limitations.} (1) The 9.2$\times$ throughput gap between OrchKvCache and GPU-only is dominated by Python-level block reconstruction and scheduling overhead, not attention collection frequency (Table~\ref{tab:sampling}); integrating the scheduler into FlashAttention's tiled loop would eliminate this bottleneck. (2) The prefetch scheduler uses EMA-based prediction; cross-layer~\cite{infinigen} or query-aware~\cite{quest} signals could improve accuracy. (3) Our prototype processes requests in round-robin fashion rather than true concurrent batching; the vLLM integration experiments isolate the scheduling benefit under production continuous batching.

%% ====================================================================
\section{Conclusion}
%% ====================================================================

We have presented OrchKvCache, a tiered KV-cache management system driven by runtime attention-derived hotness. Evaluation across four models (7B GQA through 13B MHA), synthetic and realistic workloads, and the full GPU$\to$DRAM$\to$SSD$\to$DRAM$\to$GPU migration path demonstrates: \textbf{1.28--1.77$\times$ throughput improvement} over FIFO offloading with \textbf{139--597$\times$ fewer data migrations}; \textbf{1.53$\times$} speedup under realistic variable-length distributions; and \textbf{100\% lossless} output through all three storage tiers. Integration into vLLM reveals a nuanced granularity hierarchy: block-level aggregate scoring for victim selection achieves \textbf{1.12$\times$} throughput over FIFO within vLLM's existing architecture, while true intra-request partial swap is blocked by PagedAttention's all-blocks-on-GPU constraint. This finding identifies a concrete kernel-level extension---supporting mixed GPU/CPU block tables in the attention computation---as the critical path to combining OrchKvCache's scheduling intelligence with production continuous-batching engines.
