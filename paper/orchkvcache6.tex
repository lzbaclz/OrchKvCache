...
\textbf{Heterogeneous Storage and Prefix-KV Management.} Strata~\cite{strata} pioneered NVM+SSD cross-media file systems. SPFS~\cite{spfs} stacks PM on SSD file systems. OrchFS~\cite{orchfs} introduced alignment-based write partitioning. TPP~\cite{tpp} manages CXL-attached tiered memory. IMPRESS~\cite{impress} addresses the prefix-KV \emph{Re-Prefill} bottleneck via importance-guided selective loading and KV reordering, achieving 2.8$\times$ TTFT reduction; however, it targets prefix reuse across requests (not runtime decode-phase management) and is lossy (loads only important tokens). OrchKvCache addresses the complementary decode-phase problem: \emph{runtime} placement of \emph{all} KV blocks across GPU/DRAM/SSD with attention-driven scheduling and lossless integrity.
