...
\algrenewcommand{\alglinenumber}[1]{}% ← suppress line numbers globally
\usepackage{tabularx}
...
\bottomrule
\multicolumn{4}{@{}l}{\scriptsize $^*$Lossless at 100\% capacity; lossy at reduced capacity.}\\
\end{tabularx}
\end{table}
...
Modern LLMs use the Transformer architecture~\cite{transformer} with rotary position embeddings~\cite{rope}, computing attention as:
\begin{equation}
...
\textbf{The Online Tiered KV-Cache Placement Problem.} During autoregressive decoding, the system must decide, at each step, which KV blocks to keep on GPU, which to demote to DRAM, and which to spill to SSD---\emph{without future knowledge}. We formalize this as an instance of the online paging problem~\cite{sleator-tarjan}: $n$ blocks compete for $k < n$ GPU slots; each eviction incurs a tier-dependent cost (GPU$\to$DRAM: 1.4\,$\mu$s/32\,KB at 23\,GB/s; DRAM$\to$SSD: 139\,$\mu$s at 0.23\,GB/s per block). The goal is to minimize total migration cost while preserving \emph{bit-exact} output. The key challenge is that access patterns are determined by the model's attention distribution, which shifts every decode step.
...
Each block tracks its identity, storage tier (GPU HBM, host DRAM, or SSD), hotness signals, and lifecycle state (free, allocated, active, migrating, or evicted). A per-block reader-writer lock prevents concurrent readers from observing partially-migrated data. The block payload for Qwen2.5-7B is:
\[
...
\textbf{Tier~2 (SSD)} stores one file per request with a deterministic offset layout that keeps blocks for the same (layer, head) pair contiguous:
\begin{equation}
\text{offset} = (\text{layer} \cdot n_{kv} \cdot B_{\max} + \text{head} \cdot B_{\max} + \text{idx}) \times \text{slab\_size}
\end{equation}
This layout ensures sequential write patterns for batch evictions, raising SSD write utilization from ${\sim}$4\% (single 32\,KB block) to over 41\% (8+ blocks batched).
...
\textbf{Demote path.} When the eviction policy (C4) identifies victim blocks:
(1)~the engine acquires a write lock on each victim, transitioning its state to \emph{migrating};
(2)~for GPU$\to$DRAM, an asynchronous DMA copy is issued on a dedicated CUDA stream (round-robin across $N{=}4$ streams);
(3)~for DRAM$\to$SSD, an asynchronous write is submitted to the IO thread pool---for batch operations, $k$~blocks are serialized into a contiguous staging buffer and issued as a single large write, improving SSD utilization from ${\sim}$9\% to ${\sim}$41\%;
(4)~for two-hop GPU$\to$SSD cold eviction, the engine chains steps~(2) and~(3) via an intermediate DRAM staging buffer;
(5)~upon completion, the source-tier slot is freed and the block's metadata is updated atomically under the write lock.

\textbf{Promote path.} For DRAM$\to$GPU, a single asynchronous DMA copy suffices. For SSD$\to$GPU (two-hop), the IO pool first reads the block into DRAM, then a DMA copy moves it to GPU. The block returns to \emph{active} state and the write lock is released, unblocking any waiting attention kernel.

\textbf{Atomicity and failure handling.} A per-block reader-writer lock ensures that no concurrent reader observes partially-migrated data. If a transfer fails (DMA error or I/O error), the engine rolls back: the block remains at its source tier in active state and any destination allocation is freed. Per-direction migration statistics (count, bytes, errors) are tracked for monitoring.
...
We evaluate OrchKvCache on \textbf{four models} spanning two attention architectures (GQA and MHA) and two model scales (7B and 13B). We measure: (1) end-to-end throughput and eviction efficiency under GPU memory pressure (\S\ref{sec:e2e}), (2) output quality preservation (\S\ref{sec:quality-eval}), (3) component ablation (\S\ref{sec:ablation}), (4) scheduling overhead and scalability (\S\ref{sec:prefetch-eval}), and (5) extended context up to 8K tokens with selective restore analysis (\S\ref{sec:scale}, \S\ref{sec:selective-restore}).
...
\textbf{Models.} We evaluate four models with deliberately varied KV-cache footprints.  Qwen2.5-7B~\cite{qwen2.5} and Mistral-7B~\cite{mistral} use GQA~\cite{gqa} (derived from MQA~\cite{mqa}); LLaMA-2~\cite{llama2} uses full MHA:
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/fig01_multimodel_throughput.pdf}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/fig02_multimodel_eviction.pdf}
...
\begin{figure*}[t]
\centering
\includegraphics[width=0.85\textwidth]{figures/fig04_05_combined_speedup.pdf}
...
\textbf{Finding 4: The advantage is robust across configurations.} Figure~\ref{fig:speedup-combined}(a) shows the OrchKvCache/FIFO speedup heatmap across all four models and two context lengths (2K, 4K) at budget=50\,MB with 8 concurrent requests. OrchKvCache exceeds 1.0$\times$ in every configuration where both methods complete; the sole empty cell (LLaMA-2-13B at 4K) reflects an OOM under the 50\,MB budget---precisely the extreme-pressure scenario where tiered management is most needed.

Figure~\ref{fig:speedup-combined}(b) summarizes the per-model speedup at seq=2048: as KV-cache size per token grows from 56\,KB (Qwen) to 800\,KB (LLaMA-2-13B), OrchKvCache's speedup increases from 1.38$\times$ to 1.80$\times$. This confirms that attention-driven scheduling becomes \emph{more valuable} on models with larger KV caches---precisely the regime where memory pressure is most severe.
...
\begin{tabular}{@{}llccc@{}}
\toprule
Model & Prompt & Prompt len & Match \% & Evictions \\
\midrule
Qwen2.5-7B  & short  & 152 & 100 & 0    \\
Qwen2.5-7B  & medium & 341 & 100 & 1024 \\
Qwen2.5-7B  & long   & 701 & 100 & 1024 \\
\midrule
Mistral-7B  & short  & 182 & 100 & 1024 \\
Mistral-7B  & medium & 342 & 100 & 1024 \\
Mistral-7B  & long   & 807 & 100 & 1024 \\
\bottomrule
\end{tabular}
\end{table}

Table~\ref{tab:quality} reports the results. Across all four models and all prompt lengths, \textbf{every generated token is bit-exact identical} to the GPU-Only baseline (100.00\% match rate). This holds even when hundreds to thousands of evictions and promotions occur during generation. The lossless property is by construction: all data movement uses \texttt{torch.Tensor.copy\_} (CUDA DMA for GPU$\leftrightarrow$DRAM) and standard file I/O---both data-preserving operations. The per-block \texttt{rwlock} prevents partial reads during migration.
...
Table~\ref{tab:lm-eval} and Figure~\ref{fig:lm-eval} show that OrchKvCache produces \textbf{identical accuracy} to the GPU-Only baseline on all four tasks across both models---8 out of 8 data points match exactly. This is the expected consequence of bit-exact lossless output: since every generated token is identical under greedy decoding, any downstream metric computed from that output (accuracy, F1, BLEU, etc.)\ is also identical by construction. The result provides empirical confirmation that OrchKvCache's three-tier migration introduces \emph{zero} quality degradation on real reasoning tasks, not just on synthetic token-match tests.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/fig08_lm_eval_accuracy.pdf}
\caption{Downstream task accuracy: GPU-Only vs.\ OrchKvCache on four NLP benchmarks. All bars are identical, confirming lossless quality preservation.}
\label{fig:lm-eval}
\end{figure}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/fig10_policy_sweep_heatmap.pdf}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/fig11_prefetch_dispatch.pdf}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.85\columnwidth]{figures/fig13_scheduling_scalability.pdf}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.85\columnwidth]{figures/fig12_inter_tier_bandwidth.pdf}
...
\textbf{E8: Storage bandwidth.} Figure~\ref{fig:bandwidth} confirms GPU$\leftrightarrow$DRAM saturates PCIe Gen4 at \textbf{23\,GB/s} for transfers $\geq$\,1\,MB. The 3.4$\times$ read/write asymmetry of the SSD motivates OrchKvCache's strategy of batching writes while allowing aggressive reads for promotion.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.85\columnwidth]{figures/fig09_competitive_ratio.pdf}
\caption{Empirical competitive ratio vs.\ number of blocks. OrchKvCache's EMA policy consistently achieves the lowest ratio among all online strategies, converging toward the offline optimal (Belady).}
\label{fig:competitive-ratio}
\end{figure}

\textbf{Finding.} Figure~\ref{fig:competitive-ratio} and Table~\ref{tab:cr} show that OrchKvCache's EMA policy achieves the \textbf{lowest competitive ratio} among all online strategies at every scale, converging to \textbf{1.51$\times$} OPT at 512 blocks. FIFO is 1.71--3.70$\times$ and LRU is 2.24--4.13$\times$---substantially worse. The EMA policy's advantage grows at smaller cache sizes (higher eviction pressure), precisely the regime where scheduling quality matters most.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.85\columnwidth]{figures/fig22_sampling_e2e_throughput.pdf}
\caption{End-to-end throughput vs.\ attention sampling interval $N$ on Qwen2.5-7B (seq=2048, budget=50\,MB, gen=64). Throughput is stable across all $N \geq 1$ (180--183\,tok/s), with 100\% token match in every case. $N{=}0$ (SDPA, no sampling) shows the upper bound without KV management overhead.}
\label{fig:sampling-e2e}
\end{figure}

\textbf{End-to-end validation.} We confirm the trace-simulation findings with actual inference on Qwen2.5-7B (seq=2048, budget=50\,MB, 64 generated tokens). Figure~\ref{fig:sampling-e2e} shows throughput as a function of the attention sampling interval $N$. Sweeping $N \in \{1, 2, 5, 10, 20, 50\}$, E2E throughput remains stable at 180--183\,tok/s (variation $<$\,1.6\%), eviction count is identical across all settings (512), and token match is 100\% in every case. This validates that the trace-level ``broad sweet spot'' translates directly to real-model performance, and that attention sampling frequency has negligible impact on end-to-end throughput.
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/fig19_realistic_throughput.pdf}
...
\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/fig20_realistic_eviction.pdf}
...
\begin{table}[t]
\centering
\caption{OrchKvCache at extended context lengths (tok/s, budget=50\,MB, gen=32). GPU-Only, FIFO, and OrchKvCache all use the same attention implementation per row. 8K uses SDPA (avoids eager-attention OOM). Token match is 100\% in every cell.}
\label{tab:scale}
\small
\begin{tabular}{@{}llcccc@{}}
\toprule
...
Table~\ref{tab:scale} and Figure~\ref{fig:scale-context} show that OrchKvCache operates correctly at 8K context on both GQA and MHA models, with \textbf{100\% token match} in every configuration.  OrchKvCache consistently outperforms FIFO: the advantage ranges from 1.23$\times$ (Qwen, 2K) to \textbf{2.12$\times$} (LLaMA, 8K), with the benefit \emph{growing} at longer contexts because larger KV caches amplify the cost of blind eviction.  The 8K rows use SDPA mode, demonstrating that OrchKvCache's scheduling benefits from recency/frequency signals alone when attention weights are unavailable, extending its applicability to FlashAttention-based inference stacks.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/fig16_scale_context.pdf}
\caption{OrchKvCache vs.\ FIFO throughput at 2K, 4K, and 8K context. The speedup grows with context length on both models, reaching 2.12$\times$ at 8K on LLaMA-2-7B.}
\label{fig:scale-context}
\end{figure}
...
\begin{table}[t]
\centering
\caption{Attention weight coverage (\%) by EMA-ranked non-sink blocks.  Top-$K$\% of blocks ranked highest by EMA score; coverage = fraction of total non-sink attention weight captured.  Random baseline for comparison.}
\label{tab:selective}
\footnotesize
\setlength{\tabcolsep}{9pt}
\begin{tabular}{@{}llrrrrr@{}}
\toprule
Model & Rank & 1\% & 5\% & 10\% & 30\% & 50\% \\
\midrule
\multirow{2}{*}{Qwen2.5-7B}
...
Table~\ref{tab:selective} and Figure~\ref{fig:selective-restore} show that even the \textbf{top-1\%} of blocks ranked by OrchKvCache's EMA score---a single block of 16 tokens out of ${\sim}$64---captures \textbf{100\%} of the non-sink attention weight.  Random ranking at the same budget captures only 2\%.  This extreme concentration means a kernel-level selective restore would need to fetch \emph{at most a few blocks} to cover all attention-critical data; the remaining 99\% of blocks could be deferred or skipped entirely, reducing restore volume by up to \textbf{100$\times$} with zero quality loss.

\begin{figure}[t]
\centering
\includegraphics[width=\columnwidth]{figures/fig17_selective_restore.pdf}
\caption{Attention weight coverage: EMA-ranked vs.\ random block selection (excluding sink). EMA achieves 100\% coverage at every budget, while random requires 50--90\% of blocks to reach comparable coverage.}
\label{fig:selective-restore}
\end{figure}
...
\caption{vLLM victim-selection strategies across memory-pressure levels. Under tight memory (gpu\_util=0.20), progress-aware scoring achieves 1.08--1.15$\times$ over FIFO; under relaxed memory ($\geq$0.25), all strategies converge.}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.85\columnwidth]{figures/fig18_infinigen_throughput.pdf}
\caption{InfiniGen throughput reproduction on OPT-1.3B (FlexGen offloading). InfiniGen's speedup grows with prompt length; H2O is faster but lossy.}
\label{fig:infinigen-thr}
\end{figure}

Figure~\ref{fig:infinigen-thr} visualises the throughput trend: InfiniGen's benefit grows with prompt length as more KV data must be offloaded, while H2O achieves higher absolute throughput at the cost of output quality.  Note that InfiniGen and OrchKvCache target different frameworks (FlexGen vs.\ HuggingFace/vLLM) and different bottlenecks: InfiniGen reduces \emph{prefetch volume} by predicting which KV blocks to load from CPU; OrchKvCache reduces \emph{migration frequency} by keeping hot blocks on GPU and batching cold-block I/O to SSD.  The two optimizations are orthogonal and could be combined: InfiniGen's speculative cross-layer prefetcher driving OrchKvCache's three-tier migration engine.
...
\textbf{KV-Cache Management.} vLLM~\cite{vllm} introduced PagedAttention for KV-cache paging. FlexGen~\cite{flexgen} formulated offline three-tier offloading. InfiniGen~\cite{infinigen} pioneered cross-layer prefetching with speculative KV selection; we provide a detailed comparison in \S\ref{sec:infinigen-comparison}. AttentionStore~\cite{attentionstore} and CacheBlend~\cite{cacheblend} reuse prefix KV caches across multi-turn conversations and RAG pipelines; ContiguousKV~\cite{contiguouskv} aligns KV pruning granularity with I/O operations to eliminate read amplification. vTensor~\cite{vtensor} provides virtual tensor management. Mooncake~\cite{mooncake} deploys datacenter-scale KV-cache pooling. Infinite-LLM~\cite{infinitellm} distributes KV-cache across GPU clusters. OrchKvCache adds \emph{attention-driven, per-block classification} with heterogeneous IO adaptation.
...
\textbf{LLM Serving Systems.} Orca~\cite{orca} introduced continuous batching. DistServe~\cite{distserve} and Splitwise~\cite{splitwise} disaggregate prefill and decode. Sarathi-Serve~\cite{sarathi} uses chunked prefill. FlashAttention~\cite{flashattn,flashattn2} provides IO-aware tiled attention. SGLang~\cite{sglang} optimizes multi-call programs. Speculative decoding~\cite{speculative} reduces latency via draft-verify parallelism. Megatron-LM~\cite{megatron} enables multi-GPU tensor parallelism. DeepSpeed-Inference~\cite{deepspeed-inf} and DeepSpeed~\cite{deepspeed} support heterogeneous inference at trillion-parameter scale. ZeRO~\cite{zero} optimises training memory via partitioning.

\textbf{Model Compression.} AWQ~\cite{awq} applies activation-aware weight quantization for on-device deployment. KIVI~\cite{kivi} quantizes KV caches asymmetrically. These are orthogonal to OrchKvCache's lossless migration and could be composed with it.

\textbf{Heterogeneous Storage and Prefix-KV Management.} Strata~\cite{strata} pioneered NVM+SSD cross-media file systems. SPFS~\cite{spfs} stacks PM on SSD file systems. OrchFS~\cite{orchfs} introduced alignment-based write partitioning. TPP~\cite{tpp} manages CXL-attached tiered memory. IMPRESS~\cite{impress} addresses the prefix-KV \emph{Re-Prefill} bottleneck via importance-guided selective loading, achieving 2.8$\times$ TTFT reduction; ContiguousKV~\cite{contiguouskv} further improves Re-Prefill by 3.85$\times$ via granularity-aligned I/O. However, both target prefix reuse (not runtime decode-phase management) and are lossy. OrchKvCache addresses the complementary decode-phase problem: \emph{runtime} placement of \emph{all} KV blocks across GPU/DRAM/SSD with attention-driven scheduling and lossless integrity.
...
\begin{figure}[t]
\centering
\includegraphics[width=0.85\columnwidth]{figures/fig15_overhead_breakdown.pdf}
\caption{Per-step overhead breakdown. \texttt{build\_past\_kv} (KV reconstruction) dominates at 59.5\%; model.forward is only 23.6\%.}
\label{fig:overhead-breakdown}
\end{figure}

\textbf{Level 1: Component breakdown.} Table~\ref{tab:overhead-breakdown} and Figure~\ref{fig:overhead-breakdown} reveal that \texttt{build\_past\_kv()} accounts for \textbf{59.5\%} of per-step time: per-step GPU tensor allocation ($28{\times}2$ \texttt{torch.zeros} calls, ${\sim}$28\,ms) plus per-block Python slice copies (${\sim}$13\,ms). Model.forward itself takes only 19\,ms---nearly identical to GPU-only (16--19\,ms).
...
\textbf{Multi-GPU extension.} OrchKvCache's per-block tier management naturally extends to multi-GPU configurations. Under tensor parallelism~\cite{megatron} (TP=2), each GPU rank runs an independent OrchKvCache instance managing its own KV head partition---requiring only the \texttt{g\_sys} singleton to be refactored into per-instance handles. A more aggressive design treats a second GPU as an intermediate storage tier (GPU$_0$ HBM $\to$ GPU$_1$ HBM $\to$ DRAM $\to$ SSD), exploiting NVLink's 600\,GB/s bidirectional bandwidth as a low-latency warm tier between HBM and DRAM. We leave this four-tier disaggregated architecture to future work.
...
\begin{tabular}{@{}rcrrc@{}}
\toprule
$N$ & Acc. & tok/s & Speedup & Match \\
\midrule
0 (SDPA) & --- & 1{,}658 & 9.21$\times$ & --- \\
1 (every) & 0.58 & 180 & 1.00$\times$ & baseline \\
5 & 0.81 & 182 & 1.01$\times$ & 100\% \\
10 & 0.81 & 183 & 1.01$\times$ & 100\% \\
50 & 0.64 & 183 & 1.02$\times$ & 100\% \\
\bottomrule
\end{tabular}
\end{table}
...
A systematic overhead analysis decomposes the throughput gap into three independent factors: batching strategy (4--8$\times$, continuous batching vs.\ sequential), framework overhead (35--45\%, Python KV reconstruction), and scheduling-loop implementation (23--49\,ms Python vs.\ $<$\,40\,$\mu$s C/CUDA). The scheduling \emph{algorithm}---attention-driven classification---is effective (139--597$\times$ fewer migrations, 1.12$\times$ in vLLM); the \emph{implementation} overhead of the Python scheduling loop currently dominates when framework overhead is equalised. Porting this loop to C/CUDA, following the same pattern as the core classifier, is the concrete path to combining OrchKvCache's scheduling intelligence with production continuous-batching engines. Together with prefix-KV systems such as IMPRESS~\cite{impress}---which optimises the \emph{Re-Prefill} loading phase---OrchKvCache completes the full KV-cache lifecycle: IMPRESS handles prefix loading; OrchKvCache manages the runtime decode-phase tier placement; and both leverage the same fundamental property of attention skewness.
...
\bibitem{speculative} Y.~Leviathan et~al., ``Fast Inference from Transformers via Speculative Decoding,'' in \textit{Proc.\ ICML}, 2023.

\bibitem{awq} J.~Lin et~al., ``AWQ: Activation-aware Weight Quantization for On-Device LLM Compression and Acceleration,'' in \textit{Proc.\ MLSys}, 2024.
