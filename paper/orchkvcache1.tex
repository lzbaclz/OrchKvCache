...
\begin{abstract}
The Key-Value (KV) cache is the dominant memory bottleneck in large language model (LLM) inference: it grows linearly with context length, and a single LLaMA-2-7B request at 128K tokens requires 64\,GB---4.6$\times$ the model's own weight. Existing KV-cache management systems fall into three categories, each with fundamental drawbacks: (i) \emph{hotness-agnostic swapping} (vLLM) treats all blocks identically, evicting via FIFO; (ii) \emph{offline, coarse-grained offloading} (FlexGen) moves entire layers using a pre-computed plan; and (iii) \emph{lossy eviction} (H2O, ScissorHands) permanently discards cold tokens.

We present \textbf{OrchKvCache}, a runtime system that manages KV blocks across GPU HBM, host DRAM, and SSD, driven by online attention-based hotness classification. OrchKvCache contributes three techniques. \textbf{(1) Attention-driven hot-cold classification.} An EMA-smoothed, multi-signal scoring function with watermark-adaptive thresholds classifies every KV block into Hot, Warm, or Cold categories each decode step. \textbf{(2) Multi-granularity IO adaptation.} Batch cold evictions are aligned to SSD 32\,KB blocks, boosting SSD write utilization from 4\% to over 41\%. \textbf{(3) Prefetch-driven compute-transfer pipeline.} A three-stage overlap hides migration latency behind useful work at $\leq$5.9\,$\mu$s overhead.

End-to-end evaluation on \textbf{four models} (Qwen2.5-7B, Mistral-7B, LLaMA-2-7B, LLaMA-2-13B) spanning GQA and MHA architectures with A100-80GB GPUs demonstrates that block-level attention-driven scheduling achieves \textbf{1.28--1.77$\times$ throughput improvement} over FIFO offloading, with the advantage scaling with per-token KV-cache size (56\,KB to 800\,KB). Under realistic variable-length workloads, OrchKvCache maintains \textbf{1.53$\times$} speedup while reducing unnecessary migrations by \textbf{123--374$\times$}. The full three-tier path (GPU$\to$DRAM$\to$SSD$\to$DRAM$\to$GPU) is validated with \textbf{100\% lossless} integrity. Integration into vLLM's scheduler further reveals that migration \emph{granularity}---block-level vs.\ request-level---is the critical factor determining scheduling effectiveness.
\end{abstract}
...
% [insert background figure: KV-cache size vs. sequence length]
...
% [insert Table 1: KV cache sizes]
% [insert Table 2: max batch size on A100-80GB]
...
% [insert Table 3: attention distribution statistics]
% [insert figure: attention CDF]
% [insert figure: attention sink]
...
% [insert Table 4: per-tier bandwidth]
% [insert figure: SSD bandwidth utilization]
...
% [insert architecture figure: storage hierarchy]
...
% [insert architecture figure: system architecture]
...
% [insert figure: pipeline timeline]
