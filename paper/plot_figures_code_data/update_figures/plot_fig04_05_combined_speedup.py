#!/usr/bin/env python3
"""
Fig 04+05 combined: (a) Speedup heatmap, (b) Per-model speedup bar.
Data:   multimodel_e2e_4models.json  (same directory)
Output: fig04_05_combined_speedup.pdf/png (same directory)
"""
...
def main():
    ...
    # --- (a) Heatmap: models x context lengths ---
    ...
    ax_bar.set_xticklabels([_short(m) for m in models], rotation=20, ha="right")
    ...
    for ext in ("pdf", "png"):
        fig.savefig(HERE / f"fig04_05_combined_speedup.{ext}")
    plt.close(fig)
    print(f"Saved fig04_05_combined_speedup.pdf/png to {HERE}")
