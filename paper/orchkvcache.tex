...
End-to-end evaluation on \textbf{four models} (Qwen2.5-7B, Mistral-7B, LLaMA-2-7B, LLaMA-2-13B) spanning GQA and MHA architectures with A100-80GB GPUs demonstrates that OrchKvCache achieves \textbf{1.28--1.77$\times$ throughput improvement} over FIFO offloading, with the advantage scaling with per-token KV-cache size (56\,KB to 800\,KB). Under realistic variable-length workloads (ShareGPT-like distribution), OrchKvCache maintains \textbf{1.53$\times$} speedup while reducing unnecessary migrations by \textbf{123--374$\times$}. The full three-tier path (GPU$\to$DRAM$\to$SSD$\to$DRAM$\to$GPU) is validated end-to-end with \textbf{100\% lossless} data integrity across all models.
\end{abstract}
...
\begin{table}[t]
\centering
\caption{Summary of key experimental findings across four models.}
\label{tab:summary}
\small
\begin{tabular}{@{}llr@{}}
\toprule
...
\midrule
E2E Throughput  & OrchKv vs FIFO        & \textbf{1.28--1.77$\times$} \\
E2E Evictions   & Migration reduction    & \textbf{139--597$\times$} \\
E2E Latency     & TPOT stability         & $\pm$0.1\% vs 7\% \\
Quality         & Token match (4 models) & \textbf{100.00\%} \\
Realistic load  & ShareGPT-like speedup  & \textbf{1.02--1.53$\times$} \\
Realistic load  & Eviction reduction     & \textbf{123--374$\times$} \\
SSD tier        & 3-tier round-trip      & \textbf{100.00\% lossless} \\
Ablation        & Throughput gain        & +22--63\% \\
Classification  & Optimal $\alpha$       & $\geq$0.7 \\
Prefetch        & Saturation budget      & $K{=}8$, 5.9\,$\mu$s \\
Scalability     & 4,096 blocks           & 38\,$\mu$s, P99 $<$ 60\,$\mu$s \\
Bandwidth       & GPU$\leftrightarrow$DRAM & 23\,GB/s \\
\bottomrule
\end{tabular}
\end{table}
...
\textbf{Limitations.} (1) Our HuggingFace-based prototype processes requests sequentially; integration with vLLM's continuous batching would amplify capacity extension. We have validated the scheduling principle within vLLM's scheduler (\S\ref{sec:realistic}); full C/CUDA core integration is future work. (2) The prefetch scheduler uses EMA-based prediction; incorporating cross-layer~\cite{infinigen} or query-aware~\cite{quest} signals could improve accuracy. (3) The eager-attention overhead for score collection dominates the throughput gap vs GPU-only; sampling every $N$ steps or using lightweight proxies would reduce this to $<$15\%.

%% ====================================================================
\section{Conclusion}
%% ====================================================================

We have presented OrchKvCache, a tiered KV-cache management system driven by runtime attention-derived hotness. Evaluation across four models (7B GQA through 13B MHA), synthetic and realistic workloads, and the full GPU$\to$DRAM$\to$SSD$\to$DRAM$\to$GPU migration path demonstrates: \textbf{1.28--1.77$\times$ throughput improvement} over FIFO offloading with \textbf{139--597$\times$ fewer data migrations} on uniform workloads; \textbf{1.53$\times$} speedup under ShareGPT-like variable-length distributions; \textbf{100\% lossless} output through all three storage tiers; and only 38\,$\mu$s scheduling overhead at 4,096 blocks. These results establish that attention-driven hotness classification is both feasible and increasingly valuable as models grow and context windows expand.
